...
FIXED VERSION: Corrects label encoding to match model training (1=LEFT, 2=RIGHT)
"""
...
def extract_mi_epochs(eeg_data, timestamps, events_df, fs=250):
    ...
    for idx, event in cue_events.iterrows():
        ...
        # FIXED: Use correct label encoding (1 for left, 2 for right)
        label = 1 if event['cue_class'] == 'left' else 2
    ...
    print(f"  Left (class 1): {labels.count(1)}")
    print(f"  Right (class 2): {labels.count(2)}")
    
    return np.array(epochs), np.array(labels), epoch_info

def create_mi_dataset(epochs, labels, epoch_info, output_dir='test_data/mi_dataset_fixed'):
    """Create dataset in T-005 MI format with correct labels"""
    ...
    for i, (epoch, label, info) in enumerate(zip(epochs, labels, epoch_info)):
        ...
        cue_idx = int(0.5 * fs)
        event_id[cue_idx] = str(label)  # Already 1 for left, 2 for right
    ...
    print(f"  Label encoding: 1=LEFT, 2=RIGHT (matching model training)")
...
def prepare_features_for_model(epochs, model_dict):
    ...
    if isinstance(model_dict, dict) and 'csp' in model_dict and model_dict['csp'] is not None:
...
def test_with_model(features, labels, model_path):
    ...
    if isinstance(model, dict):
        ...
        model_dict = model  # Keep the full dict for CSP access
        model = actual_model
    else:
    ...
    # Check if model expects binary (0/1) or multi-class (1/2) labels
    if hasattr(model, 'classes_'):
        print(f"Model classes: {model.classes_}")
        if len(model.classes_) == 2 and 0 in model.classes_:
            # Binary classifier expecting 0/1
            print("Converting labels to binary (0/1) for model")
            labels_for_model = labels - 1  # Convert 1/2 to 0/1
        else:
            # Model expects 1/2 format
            labels_for_model = labels
    else:
        # Default: assume model expects same format as our labels
        labels_for_model = labels
    
    # Make predictions
    predictions = model.predict(features)
    
    # Convert predictions back to our label space if needed
    if hasattr(model, 'classes_') and len(model.classes_) == 2 and 0 in model.classes_:
        # Convert predictions from 0/1 back to 1/2
        predictions_display = predictions + 1
        labels_display = labels
    else:
        predictions_display = predictions
        labels_display = labels_for_model
    
    # Calculate metrics
    accuracy = accuracy_score(labels_for_model, predictions)
    cm = confusion_matrix(labels_for_model, predictions)
    
    print(f"\nModel Performance:")
    print(f"  Accuracy: {accuracy:.2%}")
    
    # Show prediction distribution
    unique, counts = np.unique(predictions_display, return_counts=True)
    print(f"\nPrediction distribution:")
    for pred, count in zip(unique, counts):
        class_name = "LEFT" if pred == 1 else "RIGHT" if pred == 2 else f"Class {pred}"
    ...
    plt.title(f'Confusion Matrix - Test Data (Fixed Labels)\nAccuracy: {accuracy:.2%}')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.tight_layout()
    plt.savefig('test_data/model_performance_test_data_fixed.png', dpi=150)
    plt.close()
    ...
    if hasattr(model, 'predict_proba'):
        ...
        left_mask = labels_display == 1
        right_mask = labels_display == 2
        
        plt.hist(probs[left_mask, 0], bins=20, alpha=0.5, label='Left trials', color='blue')
        plt.hist(probs[right_mask, 0], bins=20, alpha=0.5, label='Right trials', color='red')
        ...
        correct = predictions == labels_for_model
        ...
        plt.scatter(range(len(predictions_display)), predictions_display, 
                   c=labels_display, cmap='coolwarm', alpha=0.6)
        plt.xlabel('Trial')
        plt.ylabel('Prediction')
        plt.yticks([1, 2], ['LEFT', 'RIGHT'])
        plt.title('Predictions vs True Labels')
        plt.colorbar(label='True Label', ticks=[1, 2])
    
    plt.tight_layout()
    plt.savefig('test_data/model_predictions_test_data_fixed.png', dpi=150)
    plt.close()
    
    print(f"\nPlots saved to test_data/")
    
    return predictions_display, accuracy, cm

def main():
    """Main processing pipeline"""
    
    print("="*60)
    print("Creating MI Dataset from Test Data (FIXED LABELS)")
    print("="*60)
    print("Label encoding: 1=LEFT, 2=RIGHT (matching model training)")
    ...
    # Create dataset in T-005 format with fixed labels
    ...
    print("="*60)
    print(f"\nKey fix applied: Labels now use 1=LEFT, 2=RIGHT")
    print(f"This matches the model training and should resolve the flip issue.")
