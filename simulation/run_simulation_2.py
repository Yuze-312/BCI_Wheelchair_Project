...
class HandImages:
    """Container for hand images to ensure they persist"""
    LEFT = None
    RIGHT = None
    SIZE = (60, 60)
    ...
    def load(cls):
        ...
        try:
            ...
            if os.path.exists(left_path) and os.path.exists(right_path):
                cls.LEFT = pygame.image.load(left_path)
                cls.RIGHT = pygame.image.load(right_path)
                # Scale images
                cls.LEFT = pygame.transform.scale(cls.LEFT, cls.SIZE)
                cls.RIGHT = pygame.transform.scale(cls.RIGHT, cls.SIZE)
                print("Hand images loaded successfully via HandImages class")
                return True
            else:
                print("Hand images not found")
                return False
        except Exception as e:
            print(f"Error loading hand images: {e}")
            return False
...
# LEFT_HAND_IMG = None  # Now using HandImages.LEFT
# RIGHT_HAND_IMG = None  # Now using HandImages.RIGHT
...
def draw_endless_world(screen, world, agent, coins, word_cue):
    ...
    # No globals needed - using HandImages class
    screen.fill(COL['bg'])
...
def draw_word_cue(screen, word_cue):
    ...
    if word_cue.cue_word == "LEFT":
        if HandImages.LEFT:
            # When thinking LEFT, show left hand on the RIGHT side of cue box
            hand_x = cue_x + cue_width - HandImages.SIZE[0] - 10
            hand_y = cue_y + 20  # Position near the top
            
            # Add subtle glow effect behind hand
            glow_surface = pygame.Surface((HandImages.SIZE[0] + 20, HandImages.SIZE[1] + 20), pygame.SRCALPHA)
            ...
            for i in range(3):
                ...
                if alpha > 0:
                    pygame.draw.ellipse(glow_surface, (*border_color, alpha), 
                                      (10 - i*3, 10 - i*3, 
                                       HandImages.SIZE[0] + i*6, HandImages.SIZE[1] + i*6))
            screen.blit(glow_surface, (hand_x - 10, hand_y - 10))
            
            # Draw the hand image
            screen.blit(HandImages.LEFT, (hand_x, hand_y))
        else:
            if DEBUG_MODE:
                print("WARNING: HandImages.LEFT is None when trying to display")
        
    elif word_cue.cue_word == "RIGHT":
        if HandImages.RIGHT:
            # When thinking RIGHT, show right hand on the LEFT side of cue box
            hand_x = cue_x + 10
            hand_y = cue_y + 20  # Position near the top
            ...
            # Draw the hand image
            screen.blit(HandImages.RIGHT, (hand_x, hand_y))
        else:
            if DEBUG_MODE:
                print("WARNING: HandImages.RIGHT is None when trying to display")
...
def main():
    ...
    # Load hand images using the HandImages class
    HandImages.load()
